        ...
        <el-dialog v-model="dialogVisible" :title="editMode ? 'Edit product' : 'Add Product'" width="60%"
            ...
            <form @submit.prevent="editMode ? updateProduct() : AddProduct()" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="relative z-0 group">
                        <input v-model="title" type="text" name="floating_title" id="floating_title"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer transition-colors"
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="relative z-0 group">
                        <select id="category" v-model="category_id"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 focus:outline-none focus:ring-0 focus:border-blue-600 cursor-pointer transition-colors">
                            <option value="" disabled selected class="bg-white">Select Category</option>
                            <option v-for="category in categories" :key="category.id" :value="category.id" class="bg-white">
                                {{ category.name }}
                            </option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="relative z-0 group">
                        <select id="brand" v-model="brand_id"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 focus:outline-none focus:ring-0 focus:border-blue-600 cursor-pointer transition-colors">
                            <option value="" disabled selected class="bg-white">Select Brand</option>
                            <option v-for="brand in brands" :key="brand.id" :value="brand.id" class="bg-white">
                                {{ brand.name }}
                            </option>
                        </select>
                    </div>
                    <div class="relative z-0 group">
                        <textarea id="message" rows="2" v-model="description"
                            class="block p-2 w-full text-sm text-gray-900 bg-transparent border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-blue-600 resize-none transition-colors"
                    ...
                    <div class="p-3 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex gap-2 overflow-x-auto py-2 px-1">
                            <div v-for="(pimage, index) in product_images" :key="pimage.id" 
                                class="relative flex-shrink-0 w-24 h-24 group transition-transform hover:scale-105">
                                <img class="w-full h-full object-cover rounded-lg shadow-sm" :src="`/${pimage.image}`" alt="">
                                <button @click="deleteImage(pimage, index)"
                                    class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 hover:bg-red-600 rounded-full text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 shadow-sm">
                                    ×
                                </button>
                            </div>
                        </div>
                ...
                <div class="flex justify-end pt-2">
                    <button type="submit"
                        class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 focus:outline-none transition-all duration-200">
                        <span class="relative">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="(product, index) in products" :key="product.id"
                                ...
                                <td class="px-4 py-3">
                                    <img v-if="product.product_images && product.product_images.length > 0" :src="`/${product.product_images[0].image}`" alt="Product Image" class="w-14 h-14 object-cover rounded" />
                                    <img v-else src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png" alt="No Image" class="w-14 h-14 object-cover rounded" />
                                </td>
                                <th scope="row"
